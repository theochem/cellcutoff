...
int box_cutoff_points(const cl::BoxSortedPoints* bsp, const double* center,
    ...
    int npoint(0);
    for (cl::BoxCutoffIterator bci(bsp, center, radius); bci.busy(); ++bci) {
      ++npoint;
      dds->push_back(bci.delta()[0]);
      dds->push_back(bci.delta()[1]);
      dds->push_back(bci.delta()[2]);
      dds->push_back(bci.distance());
      ipoints->push_back(bci.ipoint());
    }
    return npoint;
}
